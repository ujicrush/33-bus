...
def SOC_ACOPF_2D_alocation(baseMVA, NT, num_nodes, num_lines, Yp, sending_node, receiving_node, IndPV,
    ...
    # 上面这里我觉得源代码有问题，我们真正想要的语义是：在第 time 个时间点，把这些 PV 节点的功率固定成它们的最大值
    # 即我们只要对 p_n[i, time] 做约束，而不是 p_n[i, :]
    # 我觉得应该是下面这样:
    # for time in range(NT):
    #     if len(IndPV[time]) > 0:
    #         constraints.append(p_n[IndPV[time], time] == p_n_max[IndPV[time], time])
